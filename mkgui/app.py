...
from vocoder import inference as gan_vocoder
...
if os.path.isdir(VOC_MODELS_DIRT):    
    ...
    vocoders =  Enum('vocoders', list((file.name, file) for file in Path(VOC_MODELS_DIRT).glob("**/*.pt")))
...
def synthesize(input: Input) -> Output:
    ...
    if input.upload_audio_file != None:
        ...
        wav, sample_rate = sf.read(input.local_audio_file.value)
        #wav, sample_rate  = librosa.load(input.local_audio_file.value)
        sf.write(TEMP_SOURCE_AUDIO, wav, sample_rate) #Make sure we get the correct wav
    wav = dsp.denoise(wav, sample_rate)
    ...
    #wav = dsp.denoise(wav, sample_rate)
    # write and output 
    sf.write(TEMP_RESULT_AUDIO, wav, sample_rate) #Make sure we get the correct wav
